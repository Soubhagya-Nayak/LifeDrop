            ...
            {donationRequests.length === 0 ? (
                ...
                donationRequests.map((request) => (
                    <View key={request.id} style={styles.requestCard}>
                        ...
                        {/* Primary donor: generate personal QR to show hospital */}
                        {request.approved_donor_id === user.id && request.request_status === 'Donation In Progress' && (
                            <TouchableOpacity
                                style={[styles.scanButton, { backgroundColor: palette.success, marginTop: 8 }]}
                                onPress={() => router.push(`/generate-qr?requestId=${request.id}`)}
                            >
                                <QrCode size={16} color="#fff" />
                                <Text style={styles.scanButtonText}>Generate My Donor QR</Text>
                            </TouchableOpacity>
                        )}

                        {/* Navigate to the hospital once approved */}
                        {request.approved_donor_id === user.id && request.request_status === 'Donation In Progress' && (
                            <TouchableOpacity
                                style={[styles.mapButton, { marginTop: 8 }]}
                                onPress={() => router.push(
                                    `/map-route?originLat=${requestForm.latitude}&originLng=${requestForm.longitude}` +
                                    `&destLat=${request.latitude}&destLng=${request.longitude}` +
                                    `&destName=${encodeURIComponent(request.hospital_name || 'Hospital')}` +
                                    `&requestId=${request.id}`
                                )}
                            >
                                <Navigation size={16} color={palette.tint} />
                                <Text style={styles.mapButtonText}>Navigate to Hospital</Text>
                            </TouchableOpacity>
                        )}
            ...
            {requesterRequests.length === 0 ? (
                ...
                requesterRequests.map((request) => (
                    <View key={request.id} style={styles.requestCard}>
                        ...
                        {/* Hospital: view compatible nearby donors for this request */}
                        <TouchableOpacity
                            style={styles.mapButton}
                            onPress={() => router.push(
                                `/donors?requestId=${request.id}` +
                                `&hospitalLat=${request.latitude}&hospitalLng=${request.longitude}` +
                                `&hospitalName=${encodeURIComponent(request.hospital_name || 'Hospital')}`
                            )}
                        >
                            <Users size={16} color={palette.tint} />
                            <Text style={styles.mapButtonText}>View Nearby Donors on Map</Text>
                        </TouchableOpacity>
