...
exports.generateQR = async (req, res) => {
    ...
    try {
        ...
        // Only the rank-1 (primary) donor or the approved donor may generate QR
        const [rank1] = await pool.query(
            'SELECT donor_id FROM BackupDonors WHERE request_id = ? ORDER BY rank_order ASC LIMIT 1',
            [requestId]
        );
        const approvedDonorId = request[0].approved_donor_id;
        const isPrimary = (rank1.length > 0 && rank1[0].donor_id === donor_id)
            || (approvedDonorId && approvedDonorId === donor_id);

        if (!isPrimary) {
            return res.status(403).json({ msg: 'Only the primary donor can generate the donation QR code.' });
        }
