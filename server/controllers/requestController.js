...
exports.acceptRequest = async (req, res) => {
    ...
    try {
        ...
        // Use a dedicated connection to safely run a transaction
        ...
        try {
            ...
            for (let i = 0; i < participants.length; i++) {
                ...
                if (p.isNew) {
                    await connection.query(
                        'INSERT INTO BackupDonors (request_id, donor_id, rank_order) VALUES (?, ?, ?)',
                        [requestId, p.id, rank]
                    );
                } else {
                    await connection.query(
                        'UPDATE BackupDonors SET rank_order = ? WHERE id = ?',
                        [rank, p.bd_id]
                    );
                }
            }
            ...
            await connection.query(
                'UPDATE BloodRequests SET request_status = ? WHERE id = ?',
                ['Primary Donor Assigned', requestId]
            );
